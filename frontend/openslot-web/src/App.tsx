...
function ResourceForm({ token, venues, services, categories, onDone, onError }: { token: string; venues: ProviderVenue[]; services: ProviderService[]; categories: Category[]; onDone: () => void; onError: (message: string) => void }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); try { await api.createProviderResource({ venueId, name, resourceType: resourceType.trim() || 'Chỗ đặt', code: code || null, floorOrZone: floorOrZone || null, positionDescription: positionDescription || null, maxCapacity: Number(maxCapacity) }, token); onDone() } catch (e) { onError(e instanceof Error ? e.message : 'Không thể thêm chỗ đặt.') } finally { setSaving(false) } }
  return <form className="provider-form catalog-form" onSubmit={submit}>
    <h3>Thêm chỗ đặt (Sân, Bàn, Ghế, Phòng...)</h3>
    ...
    <button disabled={saving || !venues.length} className="btn btn-primary rounded-pill">{saving ? 'Đang lưu...' : 'Lưu chỗ đặt'}</button>
