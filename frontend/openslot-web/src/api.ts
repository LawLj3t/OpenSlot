...
export const api = {
  ...
  activateProviderResource: (resourceId: string, token: string) => request<void>(`/provider/resources/${resourceId}/activate`, { method: 'POST' }, token),
  ...
  cancelProviderSlot: (slotId: string, token: string) => request<void>(`/provider/slots/${slotId}/cancel`, { method: 'POST' }, token),
  republishProviderSlot: (slotId: string, token: string) => request(`/provider/slots/${slotId}/republish`, { method: 'POST' }, token),
  ...
  adminReopenSlot: (slotId: string, token: string) => request<void>(`/admin/slots/${slotId}/reopen`, { method: 'POST' }, token),
  ...
  chatConversations: (token: string, role?: string) => request<Conversation[]>(`/chat/conversations${role ? `?role=${encodeURIComponent(role)}` : ''}`, {}, token),
  ...
  deleteConversation: (id: string, token: string) => request<void>(`/chat/conversations/${id}`, { method: 'DELETE' }, token),
  createSupportTicket: (payload: { userRole: string; category: string; senderEmail: string; content: string; attachmentFileName?: string | null; attachmentData?: string | null }, token?: string) => request<SupportTicket>('/support-tickets', { method: 'POST', body: JSON.stringify(payload) }, token),
  ...
  mySupportTickets: (token: string) => request<SupportTicket[]>('/support-tickets/mine', {}, token),
  mySupportTicket: (id: string, token: string) => request<SupportTicket>(`/support-tickets/mine/${id}`, {}, token),
  ...
  bulkDeleteProviders: (ids: string[], token: string) => request<{ deletedCount: number; skippedCount: number; skipped: string[] }>('/admin/providers/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) }, token),
  bulkSetProviderStatus: (ids: string[], status: number, token: string) => request<{ updatedCount: number }>('/admin/providers/bulk-status', { method: 'POST', body: JSON.stringify({ ids, status }) }, token),
  bulkDeleteCategories: (ids: number[], token: string) => request<{ deletedCount: number; skippedCount: number; skipped: string[] }>('/admin/categories/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) }, token),
  ...
  bulkDeleteUsers: (ids: string[], token: string) => request<{ deletedCount: number; skippedCount: number; skipped: string[] }>('/admin/users/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) }, token),
  bulkResolveReports: (ids: string[], token: string, resolutionNote?: string) => request<{ resolvedCount: number }>('/reports/bulk-resolve', { method: 'POST', body: JSON.stringify({ ids, resolutionNote }) }, token),
  bulkDeleteReports: (ids: string[], token: string) => request<{ deletedCount: number }>('/reports/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) }, token),
}
